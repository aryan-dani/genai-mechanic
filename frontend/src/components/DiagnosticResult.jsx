    ...
    return (
        <div className="diagnostic-result glass-panel animate-slide-in">
            <div className="result-header">
                ...
                {duration && <span className="duration-badge">{duration.toFixed(0)} ms latency</span>}
            </div>

            <div className="confidence-row">
                <div className="confidence-card">
                    <span className="conf-label">ML Prediction</span>
                    <span className="conf-value text-primary">{result.ml_evidence !== 'None' ? 'Active' : 'N/A'}</span>
                </div>
                <div className="confidence-card">
                    <span className="conf-label">RAG / Manuals</span>
                    <span className="conf-value text-primary">{result.rag_evidence !== 'None' ? 'Matched' : 'N/A'}</span>
                </div>
                <div className="confidence-card">
                    <span className="conf-label">Overall Confidence</span>
                    <span className="conf-value text-success">{result.confidence_level}</span>
                </div>
            </div>
            ...
            {result.safety_warning && result.safety_warning.toLowerCase() !== 'none' && (
            ...
            <div className="evidence-accordion">
                <div className="accordion-item" onClick={() => toggleSection('rag')}>
                    <div className="accordion-header">
                        <span><FileText size={16} /> View Service Manual Evidence (RAG)</span>
                        {expandedSection === 'rag' ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                    </div>
                    {expandedSection === 'rag' && (
                ...
                <div className="accordion-item" onClick={() => toggleSection('web')}>
                    <div className="accordion-header">
                        <span><Globe size={16} /> View Live Web Evidence</span>
                        {expandedSection === 'web' ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                    </div>
                    {expandedSection === 'web' && (
                        <div className="accordion-content">
