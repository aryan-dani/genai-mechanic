                        ...
                        try {
                            ...
                            if (data.type === 'step' || data.type === 'status') {
                                setCurrentWorkflow(prev => {
                                    ...
                                    const cleanHistory = updatedHistory.filter((val, i, arr) => i === 0 || val !== arr[i - 1]);
                                    return { ...prev, activeNode: data.node, history: cleanHistory };
                                });
                            } else if (data.type === 'complete') {
                                setCurrentWorkflow(prev => prev ? { ...prev, isComplete: true, duration: data.duration_ms } : null);
                                setMessages(prev => [...prev, {
                                    role: 'assistant',
                                    type: 'structured',
                                    data: data.data,
        ...
        <div className="chat-container glass-card">
            <div className="chat-history">
                {messages.length === 0 && (
                    <div className="empty-state">
                        ...
                        <h2>System Ready</h2>
                        <p>Describe the issue, provide DTCs, or upload telemetry to begin.</p>
                    </div>
                )}
                ...
                {messages.map((msg, idx) => (
                    <div key={idx} className={`message-wrapper ${msg.role}`}>
                        ...
                        <div className="message-content">
                            ...
                            {msg.type === 'error' && <p className="text-danger font-semibold">Error: {msg.content}</p>}
                            {msg.type === 'structured' && <DiagnosticResult result={msg.data} duration={msg.duration} />}
                        </div>
                    </div>
                ))}
            ...
            <div className="chat-input-area">
                <form onSubmit={handleSubmit} className="input-form">
                    ...
                    />
                    <button type="submit" className="send-btn" disabled={!inputValue.trim() || isProcessing}>
                        <Send size={18} />
                    </button>
                </form>
            </div>
